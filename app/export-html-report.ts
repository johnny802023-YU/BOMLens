import { companyColumnLabels, sortBomDiffsForAll, type BomAlternative, type BomDiff, type CompanyColumnKey, type DiffPrimaryType } from "./bom-logic.ts";
...
export function buildBomHtmlReport(diffs: BomDiff[], beforeName: string, afterName: string, context: ReportContext = {}) {
  ...
  const detailRows = orderedDiffs.map((diff) => `
    <tr>
      <td><span class="type ${typeTone(diff.primaryType)}">${escapeHtml(primaryLabels[diff.primaryType])}</span></td>
  ...
  const lifecycleRows = orderedDiffs.flatMap((diff) => [
    ...diff.newParts.map((part) => `<tr><td><span class="type added">新版完全新料</span></td><td>${escapeHtml(partLabel(part))}</td><td>${escapeHtml(part.manufacturerPart || "—")}</td><td>${escapeHtml(part.manufacturerName || "—")}</td><td>${joinValues(diff.after?.positions ?? [])}</td><td>${diff.after?.qty ?? 0}</td><td>${escapeHtml(primaryLabels[diff.primaryType])}</td></tr>`),
    ...diff.deletedParts.map((part) => `<tr><td><span class="type removed">新版完全移除</span></td><td>${escapeHtml(partLabel(part))}</td><td>${escapeHtml(part.manufacturerPart || "—")}</td><td>${escapeHtml(part.manufacturerName || "—")}</td><td>${joinValues(diff.before?.positions ?? [])}</td><td>${diff.before?.qty ?? 0}</td><td>${escapeHtml(primaryLabels[diff.primaryType])}</td></tr>`),
  ]).join("");
  const reviewRows = orderedDiffs.filter((diff) => diff.needsReview).map((diff) => `<tr><td>${escapeHtml(primaryLabels[diff.primaryType])}</td><td>${diff.before ? joinParts(diff.before.alternatives) : "—"}</td><td>${diff.after ? joinParts(diff.after.alternatives) : "—"}</td><td>${joinValues(diff.after?.positions ?? diff.before?.positions ?? [])}</td><td>${escapeHtml(diff.matchReason)}</td><td>${escapeHtml(diff.before?.sourceRows?.join(", ") || "—")}</td><td>${escapeHtml(diff.after?.sourceRows?.join(", ") || "—")}</td></tr>`).join("");
