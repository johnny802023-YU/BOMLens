...
在線路圖比對頁載入含可搜尋文字的 PDF 後，可直接點擊 BOM 差異表中的插件位置（例如 `U45`）。系統會在本機建立 PDF 文字索引，讓舊版與新版線路圖各自跳到對應頁面並以紅框標示。若同一位置出現多次，可使用上一筆／下一筆切換。
...
舊版與新版 PDF 預設啟用同步檢視，縮放或捲動其中一邊時另一邊會跟著對齊，也可切換成獨立檢視。旋轉 90／270 度的文字會依方向計算框選範圍，垂直拆開的插件位置也會嘗試合併。
